...
import { deleteContentItemById } from 'apiSdk/content-items';
...
function CompanyViewPage() {
  ...
  const { data, error, isLoading, mutate } = useSWR<CompanyInterface>(
    ...
    () =>
      getCompanyById(id, {
        relations: ['user', 'content_item'],
      }),
  );

  const content_itemHandleDelete = async (id: string) => {
    setDeleteError(null);
    try {
        ...
        {isLoading ? (
          ...
          <>
            ...
            <Box>
              ...
              {hasAccess('content_item', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                <Stack spacing={2} mb={8}>
                  <Flex justifyContent="space-between">
                    <Text fontSize="lg" fontWeight="bold">
                      Content Items
                    </Text>
                    <NextLink passHref href={`/content-items/create?company_id=${data?.id}`}>
                  ...
                  <TableContainer mb={4}>
                    <Table variant="simple">
                      ...
                      <Tbody>
                        {data?.content_item?.map((record) => (
                          <Tr
                            cursor="pointer"
                            onClick={() => router.push(`/content-items/view/${record.id}`)}
                            key={record.id}
                          >
                            <Td>{record.title}</Td>
                            ...
                            <Td>
                              {hasAccess('content_item', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                                <NextLink
                                  onClick={(e) => e.stopPropagation()}
                                  passHref
                                  href={`/content-items/edit/${record.id}`}
                                >
                              ...
                              {hasAccess('content_item', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
                                <IconButton
                                  onClick={(e) => {
                                    e.stopPropagation();
                                    content_itemHandleDelete(record.id);
                                  }}
                                  colorScheme="red"
                                  variant="outline"
                                  aria-label="edit"
